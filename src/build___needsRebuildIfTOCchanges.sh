...
# USE THIS BUILD SCRIPT ONLY IF YOU NEED TO PUT FOOTNOTES IN PARTS PRECEDING THE TOC, ID EST IN mailpile__img_001__above.md

# This approach would allow to put footnotes in aaa__A__mainTitle.md
# but it would build twice when the TOC changes (it takes one second anyway).
...
echo "Assembling Markdown parts and images into first step Markdown file \"$mdFilename_stepA\"..."

bash ./_aux_doPackItAll_dontCallThisOneDirectly.sh > "$mdFilename_stepA"
...
then
    ...
    then
        haveOldToc=true
        mv "$tocFileName" "${tocFileName}_old"
    else
        echo "WARNING: previous Table of Contents not found, will build a second time."
        buildAgain=true
    fi
    ...
    if $haveOldToc
    then
        if ! cmp "${tocFileName}_old" "$tocFileName" &> /dev/null
        then
            echo "The Table Of Contents \"$tocFileName\" has changed. Will build again."
            buildAgain=true
        fi
        rm "${tocFileName}_old"
    else
            echo "Table Of Contents \"$tocFileName\" created."
    fi
fi
...
then
    ...
    echo "Building again."
    echo ""
    bash $0 # Not just $0 because I'm keeping it with no execute permission so launching the other build script from keyboard is faster, no autocompletion on this one.
fi
